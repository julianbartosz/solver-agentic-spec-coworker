...
def _set_nested(obj: Dict[str, Any], keys: List[str], value: Any) -> None:
    ...
    Robustly handles cases where intermediate keys already have non-dict values
    (e.g., a list or string). In such cases, we wrap the existing value in a dict
    under a special '_value' key to preserve it while allowing nested access.
    
    Bug fix: Previously, if an intermediate key contained a list (e.g., from an
    earlier parse like `constraints.extra=[...]`), the next iteration would crash
    with "'list' object has no attribute 'get'" when trying to check `key not in obj`.
    """
    for key in keys[:-1]:
        if key not in obj:
            obj[key] = {}
        elif not isinstance(obj[key], dict):
            # Handle non-dict intermediate values by wrapping them
            # This preserves the original value while allowing nested paths
            existing_value = obj[key]
            obj[key] = {"_value": existing_value}
        obj = obj[key]
    
    # Handle final key assignment
    # If final destination is not a dict but we're trying to set a key, wrap it
    if not isinstance(obj, dict):
        # This shouldn't happen with the above fix, but guard against it
        return
    obj[keys[-1]] = value
