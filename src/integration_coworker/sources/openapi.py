...
class OpenAPISource:
    ...
    def _looks_like_openapi_json(self, content: str) -> bool:
        ...
        Handles edge cases like Stripe API where "openapi" field is at the end
        of a large file (line 60960 in stripe_api.json).
        """
        content_lower = content[:2000].lower()
        ...
        # For large JSON files, check the tail (where openapi field often is)
        # Also check a sample from the end of the file
        if len(content) > 5000:
            tail_lower = content[-20000:].lower() if len(content) > 20000 else content[2000:].lower()
            if '"openapi":' in tail_lower or '"swagger":' in tail_lower:
                return True
        
        # Structural heuristics: if it has components/paths/info, it's likely OpenAPI
        # This helps with specs where openapi field is buried deep
        if '"components":' in content_lower and ('"schemas":' in content_lower or '"paths":' in content[:10000].lower()):
            return True
    ...
    def _try_parse_openapi(self, content: str, uri: str) -> tuple:
        ...
        try:
            ...
            # G-03: Native OpenAPI/Swagger is deterministic
            data["_conversion_quality"] = "deterministic"
            data["_conversion_warnings"] = []
    ...
    def _graphql_to_pseudo_openapi(self, graphql_doc, uri: str) -> dict:
        ...
        spec = {
            ...
            "_source_uri": uri,
            "_conversion_quality": "deterministic",  # G-03: GraphQL schema parsing is deterministic
    ...
    def _asyncapi_to_pseudo_openapi(self, asyncapi: dict, uri: str) -> dict:
        ...
        spec = {
            ...
            "_source_uri": uri,
            "_conversion_quality": "deterministic",  # G-03: AsyncAPI parsing is deterministic
            "_conversion_warnings": [],
            "info": {
    ...
    def _html_to_pseudo_openapi(self, html_doc, uri: str) -> dict:
        ...
        # G-03: HTML extraction may use LLM/heuristics - mark as non-deterministic
        warnings = [
            "Converted from HTML documentation using pattern extraction",
            "Endpoint definitions may be incomplete or inferred",
        ...
        spec = {
            ...
            "_parsed_from": "html",
            "_source_uri": uri,
            "_conversion_quality": "llm_assisted",  # G-03: HTML parsing uses heuristics/LLM
            "_conversion_warnings": warnings,
            "info": {
    ...
    def _pdf_to_pseudo_openapi(self, pdf_doc, uri: str) -> dict:
        ...
        # G-03: PDF extraction is best-effort using OCR/LLM - highly non-deterministic
        warnings = [
            "Converted from PDF documentation using OCR/LLM extraction",
            "Endpoint definitions are inferred and may be inaccurate",
            "Request/response schemas may be incomplete",
        ...
        spec = {
            ...
            "_parsed_from": "pdf",
            "_source_uri": uri,
            "_conversion_quality": "best_effort",  # G-03: PDF parsing is best-effort
            "_conversion_warnings": warnings,
            "info": {
    ...
    def _extract_metadata(self, spec: dict) -> dict:
        ...
        return {
            ...
            # G-03: Include conversion quality in metadata for downstream visibility
            "conversion_quality": spec.get("_conversion_quality", "unknown"),
            "conversion_warnings": spec.get("_conversion_warnings", []),
        }
