"""
Fixed-width file source handler with confidence-gated detection and TRUE STREAMING.

Implements the SpecSource and StreamingSpecSource protocols for fixed-width files:
...
- TRUE STREAMING for large files (v2.1)
...
Streaming Architecture (v2.1):
    ...
    - parse_from_handle(): TRUE streaming schema inference
      ...
      - Infers boundaries from first N rows (configurable)
...
SCHEMA_SAMPLE_ROWS = 100    # Number of rows to sample for inference
...
class FixedWidthSource:
    ...
    def detect_from_handle(
        ...
        """
        Detect if content is fixed-width format using sample from handle.
        ...
        # Sample first 64KB for detection
        ...
        try:
            ...
            config = get_config()
            result = detect_with_confidence(sample_str, uri, config)
            
            logger.debug(
                f"FixedWidthSource streaming detection for {uri}: "
                f"matched={result.matched}, confidence={result.confidence:.2f}"
            )
    ...
    def parse_from_handle(
        ...
        """
        Parse fixed-width content with TRUE STREAMING.
        
        This method NEVER loads the entire file into memory.
        It reads line-by-line and samples first N rows for inference.
        
        Schema inference:
            1. Sample first N lines (default 100) for boundary detection
            2. Count total rows by iterating (without storing)
            3. Use existing inference algorithm on sample
        ...
        try:
            ...
            with handle.open_text() as text_stream:
                for line in text_stream:
                    # Strip line ending but preserve content
                    clean_line = line.rstrip('\r\n')
                    
                    # Sample first N lines
                    if len(sample_lines) < SCHEMA_SAMPLE_ROWS:
                        sample_lines.append(clean_line)
                    
                    if clean_line:  # Count non-empty lines
                        row_count += 1
            ...
            if not sample_lines:
                return ParsedSpec(
                    ...
                    source_uri=uri,
                    data=None,
                    errors=["No lines found in fixed-width file"],
                )
            
            # Use sampled content for inference
            sample_content = '\n'.join(sample_lines)
            
            # Run detection for metadata
            detection_result = detect_with_confidence(sample_content, uri, config)
            
            # Run inference on sample
            inference_result = infer_schema(sample_content, config=config)
            ...
            if not inference_result.is_valid():
                return ParsedSpec(
                    ...
                    metadata={
                        "streaming": True,
                        "sample_rows_used": len(sample_lines),
                        "detection": {
                            "matched": detection_result.matched,
                            "confidence": detection_result.confidence,
                        },
            ...
            logger.error(f"Fixed-width streaming parse error: {e}", exc_info=True)
            return ParsedSpec(
                source_type=SourceType.FILE,
                source_uri=uri,
                data=None,
