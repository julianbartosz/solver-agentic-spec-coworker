...
# =============================================================================
# V24-001 FIX: SIGTERM Handler for Graceful Shutdown
# Python's default SIGTERM behavior can be ignored when running in pipelines
# (e.g., `timeout $T python ... | tee log`). This explicit handler ensures
# that SIGTERM actually terminates the process.
# =============================================================================
def _sigterm_handler(signum: int, frame) -> None:
    """Handle SIGTERM by logging and exiting gracefully.
    
    This allows timeout(1) to work properly when running CLI in shell pipelines.
    Without this handler, Python may ignore SIGTERM and the timeout -k flag
    is the only way to kill the process (with SIGKILL).
    """
    logger = logging.getLogger("integration_coworker")
    logger.warning("Received SIGTERM signal - shutting down gracefully")
    
    # Attempt graceful shutdown of any running resources
    try:
        from integration_coworker.shutdown import ShutdownManager
        ShutdownManager.request_shutdown()
    except ImportError:
        pass  # ShutdownManager not available, skip
...
from integration_coworker.logging_config import configure_logging

# Configure logging
_logger = logging.getLogger("integration_coworker")

# Track logging configuration to avoid duplicate handlers while still allowing
# later calls to increase verbosity (e.g., callback initializes WARNING, command upgrades to DEBUG).
_LOGGING_STATE = {
    "configured": False,
    "level": None,
    "json_output": None,
...
def _setup_logging(verbose: bool = False, json_logs: Optional[bool] = None) -> None:
    """
    ...
    Args:
        ...
        json_logs: Force JSON output when True. If None, uses JSON_LOGS env var.
    """
    level = logging.DEBUG if verbose else logging.WARNING
    json_output = os.environ.get("JSON_LOGS", "0") == "1" if json_logs is None else json_logs
    state = _LOGGING_STATE

    # Skip reconfiguration when already configured at an equal or more verbose level
    # with the same formatter selection. This preserves a DEBUG configuration even if
    # a later call requests WARNING.
    if state["configured"]:
        if state["json_output"] == json_output and level >= (state["level"] or logging.WARNING):
            return
    ...
    # Reuse the integration_coworker handler for noisy dependencies when verbose
    handler = None
    integration_logger = logging.getLogger("integration_coworker")
    if integration_logger.handlers:
        handler = integration_logger.handlers[0]

    dependency_loggers = ["langchain", "langgraph", "openai", "httpx"]
    for logger_name in dependency_loggers:
        lg = logging.getLogger(logger_name)
        lg.setLevel(logging.INFO if verbose else logging.WARNING)
        if handler and not lg.handlers:
...
def _configure_logging_once() -> None:
    """Configure logging at CLI startup for all commands.

    Commands that need more verbosity still call _setup_logging(verbose=True),
    which upgrades the level without adding duplicate handlers.
    
    Also runs startup validation if INTEGRATION_COWORKER_FAIL_FAST=1.
    """
    _setup_logging(verbose=False)
    
    # Startup validation (fail-fast mode)
    # When INTEGRATION_COWORKER_FAIL_FAST=1, validate all critical config and
    # exit immediately if misconfigured. This prevents accepting work with
    # broken config.
    if os.environ.get("INTEGRATION_COWORKER_FAIL_FAST", "0") == "1":
        try:
            from integration_coworker.config.startup import validate_startup_config
            result = validate_startup_config(fail_fast=True)
            # Log config summary if verbose
            if _logger.isEnabledFor(logging.DEBUG):
                _logger.debug("Startup config validated: %d warnings", len(result.warnings))
        except SystemExit:
            # validate_startup_config(fail_fast=True) calls sys.exit on errors
            raise
        except ImportError:
            _logger.warning("Startup validation module not available, skipping fail-fast checks")
...
def run_integration(
    ...
    file_input: List[Path] = typer.Option(
        ...
        "--file",
        "-f",
        help="Data file input (CSV, Excel, fixed-width). Can be specified multiple times. Auto-detected by extension.",
    ),
    ...
    skip_hitl: bool = typer.Option(
        False,
        "--skip-hitl",
        help="V3: Skip HITL approval gate (auto-approve changes). For demos/CI only.",
    ),
    force_write: bool = typer.Option(
        False,
        "--force-write",
        help=(
            "V42-004: Allow repo writes even if sandbox validation fails.\n"
            "WARNING: Bypasses code quality protection. Use only for:\n"
            "  - Debugging generated code manually\n"
            "  - CI pipelines with external validation\n"
    ...
    profile: str = typer.Option(
        ...
        "--profile",
        help=(
            "V5: Codegen profile controlling quality gates and self-review:\n"
            "  'production' (default) - Full quality gates + LLM self-review pass\n"
            "  'development' - Fast iteration, skips self-review"
        ),
    ),
    no_discover: bool = typer.Option(
        ...
        "--no-discover",
        help=(
            "Slice 1: Disable spec auto-discovery even if DISCOVERY_ENABLED=true.\n"
            "Use this to force explicit --spec-ref requirement for automation."
        ),
    ),
    ...
    Supports both API specifications (OpenAPI/AsyncAPI) and data files (CSV/Excel/fixed-width).
    
    Examples:
        # Single API spec (uses production profile with self-review by default)
        ...
        integration-coworker run --file ./transactions.csv --task "Parse daily transactions"
        
        # Parse an Excel file
        integration-coworker run --file ./report.xlsx --task "Import Excel report data"
        
        # Fixed-width file with PDF guide
        integration-coworker run --file ./data.txt --guide ./layout.pdf --task "Parse bank statement"
        
        # Mixed: API spec + data file
        integration-coworker run -s ./api.yaml --file ./upload.csv --task "Upload data via API"
        ...
        # Fast iteration without self-review (development profile)
        integration-coworker run -s ./api.yaml -t "Create payment" --profile development
    ...
    # =========================================================================
    # V3 FILE: Input validation - require at least one source OR discovery
    # Slice 1: Allow task-only when discovery is enabled
    # =========================================================================
    from integration_coworker.config import get_settings
    settings = get_settings()
    
    has_spec_input = bool(spec_ref or file_input or guide)
    
    # Determine effective discovery state (CLI flag overrides env var)
    discovery_available = settings.discovery_enabled and not no_discover
    
    if not has_spec_input:
        # No explicit spec provided - check if discovery is enabled
        if discovery_available:
            # Discovery enabled - allow task-only run
            if verbose:
                typer.echo("ℹ No --spec-ref provided, using spec auto-discovery", err=True)
        else:
            # Discovery disabled - require explicit spec
            hint = ""
            if no_discover:
                hint = "\n  (Discovery disabled by --no-discover flag)"
            elif not settings.discovery_enabled:
                hint = "\n  Hint: Enable spec discovery with DISCOVERY_ENABLED=true to use task-only mode"
            typer.echo(
                f"✗ Error: Must provide at least one --spec-ref, --file, or --guide input{hint}",
                err=True
            )
            raise typer.Exit(code=2)  # Exit code 2 for usage errors (Click/Typer convention)
    ...
    if guide and not guide.exists():
        typer.echo(f"✗ Error: Guide file not found: {guide}", err=True)
        raise typer.Exit(code=2)
    
    # =========================================================================
    # Guide-file pairing rules:
    # - A guide (PDF) specifies layout for fixed-width/text files
    # - If multiple --file options + one --guide: guide applies to ALL text/fixed-width files
    # - CSV/Excel files have self-describing headers; guide is ignored for them (warn user)
    # =========================================================================
    if guide and file_input:
        self_describing_exts = {'.csv', '.xlsx', '.xls'}
        guide_relevant_files = []
        guide_irrelevant_files = []
        for f in file_input:
            if f.suffix.lower() in self_describing_exts:
                guide_irrelevant_files.append(f.name)
            else:
                guide_relevant_files.append(f.name)
        
        if guide_irrelevant_files and not guide_relevant_files:
            # All files are self-describing, guide is useless
            typer.echo(
                f"⚠ Warning: --guide is typically for fixed-width files. "
                f"CSV/Excel files ({', '.join(guide_irrelevant_files)}) have self-describing headers.",
                err=True
            )
        elif guide_irrelevant_files:
            # Mixed: some files benefit from guide, some don't
            if verbose:
                typer.echo(
                    f"ℹ Note: --guide applies to: {', '.join(guide_relevant_files)}. "
                    f"Ignored for: {', '.join(guide_irrelevant_files)}",
                    err=True
                )
    
    # Build combined spec_refs list
    # File inputs get converted to absolute paths and passed through
    # Source detection in detect_and_parse_spec handles file type detection
    all_refs = list(spec_ref)
    for f in file_input:
        all_refs.append(str(f.resolve()))
    if guide:
        all_refs.append(str(guide.resolve()))
    
    if verbose:
        _logger.debug(f"Combined spec_refs: {len(all_refs)} sources")
        for ref in all_refs:
            _logger.debug(f"  - {ref}")
    # =========================================================================

    # V5: Set codegen profile via environment variable (before any imports that load profile)
    # Validate profile name
    valid_profiles = ("development", "production")
    profile_lower = profile.lower().strip()
    if profile_lower not in valid_profiles:
        typer.echo(f"✗ Invalid --profile: {profile}. Must be 'development' or 'production'.", err=True)
        raise typer.Exit(code=1)
    os.environ["CODEGEN_PROFILE"] = profile_lower
    if verbose:
        _logger.debug(f"Codegen profile: {profile_lower}")

    # V5: Set production-critical runtime defaults (from demo-final-showcase.sh)
    # These are set only if not already defined, allowing user override via env vars
    _PRODUCTION_DEFAULTS = {
        # LLM timeout to prevent indefinite hangs (default 120s)
        "IC_LLM_CALL_TIMEOUT": "120",
        # Memory protection to prevent OOM kills
        "IC_MEM_SAMPLER_ENABLED": "true",
        "IC_MAX_RSS_MB": "4096",
        # Streaming persistence reduces checkpoint size from 200MB+ to <20MB
        "STREAMING_PERSISTENCE": "true",
        # LLM caching for cost savings (24h TTL)
        "LLM_CACHE_ENABLED": "true",
        "LLM_CACHE_TTL": "86400",
        # Persist results to database (unless --dry-run)
        "PERSIST_RESULTS": "false" if dry_run else "true",
        # Live network mode - run generated tests against real APIs
        # Host allowlist is derived from OpenAPI spec servers
        "VALIDATION_PROFILE": "live",
        "ALLOW_LIVE": "1",
    }
    for key, default_value in _PRODUCTION_DEFAULTS.items():
        if key not in os.environ:
            ...
            if verbose:
                _logger.debug(f"Set default: {key}={default_value}")
    ...
    # V5: Show profile and self-review status
    if not json_output:
        self_review_status = "✓ enabled" if profile_lower == "production" else "○ disabled"
        cache_status = "✓" if os.getenv("LLM_CACHE_ENABLED") == "true" else "○"
        typer.echo(f"⚙️  Profile: {profile_lower} | Self-review: {self_review_status} | Cache: {cache_status}")
    ...
    if verbose:
        ...
        _logger.debug(f"Profile: {profile_lower} (self_review={'enabled' if profile_lower == 'production' else 'disabled'})")
        # Log runtime settings
        _logger.debug(f"Runtime: LLM_TIMEOUT={os.getenv('IC_LLM_CALL_TIMEOUT')}s, "
                      f"MEM_CEILING={os.getenv('IC_MAX_RSS_MB')}MB, "
                      f"CACHE={'on' if os.getenv('LLM_CACHE_ENABLED') == 'true' else 'off'}")
    ...
    options = IntegrationOptions(
        ...
        skip_hitl=skip_hitl,  # V3: Skip HITL approval gate
        force_write_on_sandbox_failure=force_write,  # V42-004: Force write on sandbox failure
        # Slice 1: Pass discovery state (False if --no-discover, None otherwise lets global setting apply)
        discovery_enabled=False if no_discover else None,
    )
    ...
    # P0: Ensure database is ready before any workflow (bootstrap schemas + tables)
    if not dry_run:
        try:
            ...
            if bootstrap_result["errors"]:
                _logger.warning(f"Database bootstrap warnings: {bootstrap_result['errors']}")
            elif verbose:
                _logger.debug(f"Database bootstrap: {bootstrap_result}")
        except Exception as e:
            _logger.warning(f"Database bootstrap failed (non-fatal): {e}")

    try:
        result = design_and_generate_integration(
            spec_refs=all_refs,  # V3 FILE: Use combined spec_refs + file inputs
        ...
        # Bug #16 fix: Log full traceback when verbose enabled for debugging
        if verbose:
            _logger.exception(f"Integration run failed: {e}")
        raise typer.Exit(code=1)
...
@app.command("hitl-status")
def hitl_status_cmd(
    run_id: str = typer.Argument(..., help="Run ID to check HITL status for"),
    json_output: bool = typer.Option(False, "--json", "-j", help="Output as JSON"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable debug logging"),
):
    """
    Check if a run is paused at HITL gate and show approval request.
    
    Use this to:
    - Check if a workflow is waiting for human approval
    - View the pending file changes before approving/rejecting
    - Get the interrupt payload for programmatic handling
    ...
    Examples:
        ...
        integration-coworker hitl-status run_abc123 --json
    """
    import asyncio
    _setup_logging(verbose)
    ...
    try:
        ...
        if not paused:
            if json_output:
                typer.echo(json.dumps({
                    "run_id": run_id,
                    "paused": False,
                    "message": "Workflow is not paused at HITL gate"
                }))
            else:
                typer.echo(f"Run {run_id}: Not paused at HITL gate")
                typer.echo("  The workflow may have completed, not started, or not reached the HITL gate yet.")
        ...
        if json_output:
            ...
            typer.echo(f"🛑 Run {run_id}: PAUSED at HITL gate")
            typer.echo("")
            if payload:
                typer.echo("Pending Approval Request:")
                typer.echo(f"  Gate: {payload.get('gate', 'unknown')}")
                typer.echo(f"  Provider: {payload.get('provider_code', 'unknown')}")
                
                summary = payload.get('summary', {})
                typer.echo(f"\n  File Changes:")
                typer.echo(f"    Total files: {summary.get('files', 0)}")
                typer.echo(f"    Additions:   +{summary.get('adds', 0)}")
                typer.echo(f"    Modifications: ~{summary.get('mods', 0)}")
                typer.echo(f"    Deletions:   -{summary.get('dels', 0)}")
                typer.echo(f"    Total bytes: {summary.get('total_bytes', 0)}")
                
                files = payload.get('files', [])
                if files:
                    typer.echo(f"\n  Files ({len(files)} shown):")
                    for f in files[:10]:  # Show first 10
                        typer.echo(f"    [{f.get('action', '?')}] {f.get('path', 'unknown')}")
                    if len(files) > 10:
                        typer.echo(f"    ... and {len(files) - 10} more")
                
                typer.echo("")
                typer.echo("To approve: integration-coworker hitl-resume " + run_id + " --approve")
                typer.echo("To reject:  integration-coworker hitl-resume " + run_id + " --reject")
            else:
                typer.echo("  Could not retrieve interrupt payload")
                
    except Exception as e:
        if verbose:
            _logger.exception("hitl-status failed")
        if json_output:
            typer.echo(json.dumps({"error": str(e)}))
        else:
            typer.echo(f"✗ Error checking HITL status: {e}", err=True)
        raise typer.Exit(code=1)
...
def hitl_resume_cmd(
    ...
    regenerate: bool = typer.Option(False, "--regenerate", help="Request regeneration with feedback (requires --reject)"),
    exclude: Optional[List[str]] = typer.Option(None, "--exclude", "-x", help="Files to exclude from approval"),
    json_output: bool = typer.Option(False, "--json", "-j", help="Output as JSON"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable debug logging"),
):
    """
    ...
    This command resumes a workflow that is waiting for human approval
    at the HITL gate. You must specify either --approve or --reject.
    
    The workflow will resume using LangGraph's Command(resume=...) mechanism
    with the same thread_id (= run_id) to ensure proper checkpoint restore.
    
    Examples:
        # Approve changes
        integration-coworker hitl-resume run_abc123 --approve
        
        # Approve with comment (--comment and --feedback are aliases)
        integration-coworker hitl-resume run_abc123 --approve --comment "LGTM"
        integration-coworker hitl-resume run_abc123 --approve --feedback "LGTM"
        
        # Reject changes
        integration-coworker hitl-resume run_abc123 --reject --comment "Not ready"
        
        # Reject with regeneration request (feedback required)
        integration-coworker hitl-resume run_abc123 --reject --regenerate --feedback "Use async/await pattern"
        
        # Approve but exclude specific files
        integration-coworker hitl-resume run_abc123 --approve --exclude src/temp.py
    """
    _setup_logging(verbose)
    
    # Validate approve/reject (mutually exclusive, one required)
    if not approve and not reject:
        if json_output:
            typer.echo(json.dumps({"error": "Must specify --approve or --reject"}))
        else:
            typer.echo("✗ Must specify --approve or --reject", err=True)
        raise typer.Exit(code=1)
    
    if approve and reject:
        if json_output:
            typer.echo(json.dumps({"error": "Cannot specify both --approve and --reject"}))
        else:
            typer.echo("✗ Cannot specify both --approve and --reject", err=True)
        raise typer.Exit(code=1)
    
    # Validate regenerate flag constraints
    if regenerate and approve:
        if json_output:
            typer.echo(json.dumps({"error": "--regenerate requires --reject"}))
        else:
            typer.echo("✗ --regenerate requires --reject (cannot regenerate approved changes)", err=True)
        raise typer.Exit(code=1)
    
    if regenerate and not feedback:
        error_msg = "--regenerate requires --feedback/--comment to guide regeneration"
        if json_output:
            typer.echo(json.dumps({"error": error_msg}))
        else:
    ...
    try:
        ...
        if not is_workflow_paused(run_id):
            if json_output:
                typer.echo(json.dumps({
                    "error": f"Run {run_id} is not paused at HITL gate"
                }))
            else:
                typer.echo(f"✗ Run {run_id} is not paused at HITL gate", err=True)
                typer.echo("  Use 'integration-coworker hitl-status' to check status.", err=True)
            raise typer.Exit(code=1)
        
        # Build approval decision with schema version (ADR-0010 requirement)
        ...
        decision = {
            "approved": approve,
            "comment": feedback or "",
            "feedback": feedback or "",  # Same value for review_gate compatibility
            "overrides": {},
        ...
        if not json_output:
            ...
            if regenerate:
                action = "Rejecting + requesting regeneration for"
            typer.echo(f"🔄 {action} changes for run: {run_id}")
            if feedback:
                typer.echo(f"   Feedback: {feedback}")
            if exclude:
                typer.echo(f"   Excluding: {', '.join(exclude)}")
            if regenerate:
                typer.echo(f"   🔄 Regeneration requested")
            typer.echo("")
        
        # Resume with the decision
        # CRITICAL: Uses thread_id == run_id contract via get_thread_config
        ...
        if json_output:
            ...
            output = {
                "run_id": run_id,
                "decision": "approved" if approve else ("rejected+regenerate" if regenerate else "rejected"),
                "regenerate": regenerate,
                "completed": "persist_run_outcome" in (result.completed_steps or []),
            ...
            if approve:
                typer.echo("✓ Changes approved and applied")
            elif regenerate:
                typer.echo("✓ Changes rejected, regeneration requested")
            else:
                typer.echo("✓ Changes rejected (no writes performed)")
        ...
        if verbose:
            _logger.exception("hitl-resume failed")
        if json_output:
            typer.echo(json.dumps({"error": str(e)}))
        else:
            typer.echo(f"✗ Resume failed: {e}", err=True)
...
@app.command("kg-decay")
def apply_kg_decay(
    decay_factor: float = typer.Option(0.95, "--factor", "-f", help="Multiply confidence by this (0.95 = 5% decay)"),
    min_confidence: float = typer.Option(0.1, "--min", "-m", help="Don't decay below this threshold"),
    dry_run: bool = typer.Option(False, "--dry-run", "-n", help="Show what would be decayed without applying"),
    json_output: bool = typer.Option(False, "--json", "-j", help="Output as JSON"),
):
    """
    Apply confidence decay to unused Knowledge Graph patterns.
    
    Patterns not used in the last 30 days have their confidence reduced
    by the decay factor. This prevents stale patterns from dominating
    fresh feedback signals.
    
    P4 Feature: Confidence decay for unused patterns.
    
    Examples:
        # Apply default decay (5% reduction)
        integration-coworker kg-decay
        
        # Dry run to preview changes
        integration-coworker kg-decay --dry-run
        
        # Aggressive decay (10% reduction)
        integration-coworker kg-decay --factor 0.90
        
        # JSON output for scripting
        integration-coworker kg-decay --json
    
    Scheduling:
        # Run weekly via cron (recommended)
        0 3 * * 0 integration-coworker kg-decay --json >> /var/log/kg-decay.log 2>&1
        
        # Or use kg-maintenance for combined sync + decay
    """
    ...
    try:
        if dry_run:
            # Show what would be decayed without applying
            conn = get_connection()
            cur = conn.cursor()
            engine = get_engine_type()
            ...
            if json_output:
                output = {
                    ...
                    "would_decay": len(patterns),
                    "patterns": [
                        {
                            "key": p[0],
                            "current_confidence": p[1],
                            "projected_confidence": max(min_confidence, p[1] * decay_factor),
                            "last_used": str(p[2]) if p[2] else None,
                        }
                        for p in patterns
                    ],
                }
                ...
                typer.echo("Confidence Decay Preview (Dry Run)")
                typer.echo("=" * 60)
                typer.echo(f"Decay factor: {decay_factor} (reduces by {(1-decay_factor)*100:.0f}%)")
                typer.echo(f"Minimum threshold: {min_confidence}")
                typer.echo(f"Patterns that would be decayed: {len(patterns)}")
                typer.echo("")
                
                if patterns:
                    typer.echo(f"{'Pattern Key':<40} {'Current':>9} {'→':>3} {'New':>9}")
                    typer.echo("-" * 65)
                    for p in patterns[:20]:  # Show first 20
                        key, current, last_used = p
                        new = max(min_confidence, current * decay_factor)
                        typer.echo(f"{key[:40]:<40} {current:>9.3f}  →  {new:>9.3f}")
                    
                    if len(patterns) > 20:
                        typer.echo(f"... and {len(patterns) - 20} more patterns")
                else:
                    typer.echo("No patterns eligible for decay.")
        else:
            # Apply the decay
            decayed_count = apply_confidence_decay(
                decay_factor=decay_factor,
                min_confidence=min_confidence,
            )
            ...
            if json_output:
                ...
                if decayed_count > 0:
                    typer.echo(f"✓ Applied decay to {decayed_count} unused patterns")
                    typer.echo(f"  Factor: {decay_factor} (reduced by {(1-decay_factor)*100:.0f}%)")
                else:
                    typer.echo("✓ No patterns needed decay (all recently used or already at minimum)")

    except Exception as e:
        if json_output:
            typer.echo(json.dumps({"error": str(e), "status": "failed"}), err=True)
        else:
            typer.echo(f"✗ Error applying decay: {e}", err=True)
        raise typer.Exit(code=1)


@app.command("kg-maintenance")
def kg_maintenance(
    decay_factor: float = typer.Option(0.95, "--decay-factor", help="Decay factor for unused patterns"),
    min_confidence: float = typer.Option(0.1, "--min-confidence", help="Minimum confidence threshold"),
    skip_sync: bool = typer.Option(False, "--skip-sync", help="Skip confidence sync step"),
    skip_decay: bool = typer.Option(False, "--skip-decay", help="Skip decay step"),
    json_output: bool = typer.Option(False, "--json", "-j", help="Output as JSON"),
):
    """
    Run combined Knowledge Graph maintenance (sync + decay).
    
    This command runs both confidence synchronization and decay in a single
    operation, suitable for scheduled maintenance jobs.
    
    Steps:
    1. Sync confidence scores from accumulated feedback
    2. Apply decay to patterns not used in 30+ days
    
    Examples:
        # Full maintenance run
        integration-coworker kg-maintenance
        
        # Only sync, skip decay
        integration-coworker kg-maintenance --skip-decay
    ...
    Scheduling (recommended weekly):
        ...
        0 3 * * 0 integration-coworker kg-maintenance --json >> /var/log/kg-maintenance.log 2>&1
    """
    from integration_coworker.feedback import (
    ...
    try:
        # Step 1: Sync confidence scores
        if not skip_sync:
            if not json_output:
                typer.echo("Step 1: Syncing confidence scores...")
            
            start = time.time()
            sync_result = update_all_confidences(reason="kg_maintenance")
            sync_duration = time.time() - start
            
            results["sync"] = {
                "updated": sync_result.get("updated", 0),
                "unchanged": sync_result.get("unchanged", 0),
                "errors": sync_result.get("errors", 0),
                "duration_seconds": round(sync_duration, 2),
            }
            
            if not json_output:
                typer.echo(f"  ✓ Updated {sync_result['updated']} templates ({sync_duration:.1f}s)")
        else:
        ...
        if not skip_decay:
            if not json_output:
                typer.echo("Step 2: Applying confidence decay...")
            ...
            decay_duration = time.time() - start
            
            results["decay"] = {
                "patterns_decayed": decayed_count,
                "decay_factor": decay_factor,
                "min_confidence": min_confidence,
                "duration_seconds": round(decay_duration, 2),
            }
            
            if not json_output:
                typer.echo(f"  ✓ Decayed {decayed_count} unused patterns ({decay_duration:.1f}s)")
        else:
            results["decay"] = {"skipped": True}
        ...
        if json_output:
            ...
            typer.echo("")
            typer.echo("✓ KG maintenance complete")
        ...
        if json_output:
            ...
            typer.echo(f"✗ Maintenance failed: {e}", err=True)
        raise typer.Exit(code=1)
...
def cache_check(
    provider: Optional[str] = typer.Option(None, "--provider", "-p", help="Scope check to specific provider code"),
    repair: bool = typer.Option(False, "--repair", "-r", help="Auto-repair inconsistencies if found"),
    strict: bool = typer.Option(False, "--strict", "-s", help="Fail with exit code 1 if inconsistent"),
    json_output: bool = typer.Option(False, "--json", "-j", help="Output as JSON"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show detailed diagnostic info"),
):
    """
    Check cache-database consistency (V23-012 Production Fix).
    
    Validates that Redis LLM cache and database state are consistent.
    Detects stale cache entries that can cause slow fallback paths.
    
    Use Cases:
    - Pre-run validation before important demos
    - Debugging slow run times  
    - Automated health checks in CI/CD
    - Post-fresh-mode validation
    
    Examples:
        # Check consistency for all providers
        integration-coworker cache-check
        
        # Check specific provider
        integration-coworker cache-check --provider stripe
        
        # Check and auto-repair if inconsistent
        integration-coworker cache-check --repair
        
        # CI/CD mode: fail if inconsistent
        integration-coworker cache-check --strict --json
    """
    ...
    if repair:
        result = ensure_cache_consistency(provider, auto_repair=True)
    else:
        result = validate_cache_consistency(provider)
    
    if json_output:
        output = result.to_dict()
        output["repair_attempted"] = repair
        typer.echo(json.dumps(output, indent=2))
    else:
        if result.is_consistent:
            typer.echo("✓ Cache-database consistency: OK")
        else:
            typer.echo("✗ Cache-database consistency: ISSUES DETECTED", err=True)
        
        typer.echo(f"\n  Database State:")
        typer.echo(f"    Spec Documents: {result.db_spec_document_count}")
        typer.echo(f"    Endpoints:      {result.db_endpoint_count}")
        typer.echo(f"    Schemas:        {result.db_schema_count}")
        typer.echo(f"    Chunks:         {result.db_chunk_count}")
        
        typer.echo(f"\n  Redis Cache:")
        typer.echo(f"    Available:      {'Yes' if result.redis_available else 'No'}")
        typer.echo(f"    LLM Keys:       {result.redis_llm_key_count}")
        typer.echo(f"    Embedding Keys: {result.redis_embedding_key_count}")
        
        typer.echo(f"\n  Checkpoints:")
        typer.echo(f"    Total:          {result.checkpoint_count}")
        typer.echo(f"    Stale:          {result.stale_checkpoint_count}")
        ...
        if verbose:
            typer.echo(f"\n  Details:")
            typer.echo(f"    Checked At:     {result.checked_at.isoformat()}")
            typer.echo(f"    Provider Scope: {result.provider_code or 'all'}")
        
        if not result.is_consistent:
            if repair:
                typer.echo("\n  ℹ Auto-repair was attempted")
            else:
                typer.echo("\n  ℹ Run with --repair to auto-fix these issues")
                typer.echo("    Or use --fresh mode on next run")
    
    if strict and not result.is_consistent:
        raise typer.Exit(code=1)


@app.command("artifact-cleanup")
def artifact_cleanup(
    older_than_days: int = typer.Option(30, "--days", "-d", help="Delete artifacts older than N days"),
    dry_run: bool = typer.Option(False, "--dry-run", "-n", help="Show what would be deleted without deleting"),
    force: bool = typer.Option(False, "--force", "-f", help="Skip confirmation prompt"),
    json_output: bool = typer.Option(False, "--json", "-j", help="Output as JSON"),
):
    """
    Clean up old artifacts to free disk space (Item D Production Hardening).
    
    Deletes artifact runs older than the specified number of days.
    Uses manifest.json 'created_at' timestamps to determine age.
    ...
    Examples:
        ...
        integration-coworker artifact-cleanup
        
        # Delete artifacts older than 7 days
        integration-coworker artifact-cleanup --days 7
        
        # Force delete without confirmation
        integration-coworker artifact-cleanup --force
        
        # JSON output for scripting
        integration-coworker artifact-cleanup --json
    ...
    # Check if store is a FilesystemArtifactStore (has purge method)
    if not hasattr(store, "purge_older_than"):
        if json_output:
            typer.echo(json.dumps({"error": "Artifact store does not support purging", "status": "failed"}))
        else:
            typer.echo("✗ Artifact store does not support purging", err=True)
        raise typer.Exit(code=1)
    
    # Confirmation prompt (skip in dry-run or force mode)
    if not dry_run and not force and not json_output:
        confirm = typer.confirm(f"Delete artifact runs older than {older_than_days} days?")
        if not confirm:
            typer.echo("Aborted.")
            raise typer.Exit(code=0)
    
    # Perform cleanup
    purged_runs = store.purge_older_than(older_than_days, dry_run=dry_run)
    ...
    if json_output:
        output = {
            ...
            "count": len(purged_runs),
        }
        typer.echo(json.dumps(output, indent=2))
    else:
        if dry_run:
            typer.echo(f"[dry run] Would delete {len(purged_runs)} artifact runs older than {older_than_days} days")
            if purged_runs:
                typer.echo("Runs that would be deleted:")
                for run_id in purged_runs[:10]:  # Show first 10
                    typer.echo(f"   - {run_id}")
                if len(purged_runs) > 10:
                    typer.echo(f"   ... and {len(purged_runs) - 10} more")
        else:
            typer.echo(f"✓ Deleted {len(purged_runs)} artifact runs older than {older_than_days} days")
...
def validate_config(
    fail_fast: bool = typer.Option(
        False,
        "--fail-fast",
        help="Exit with error code if validation fails (vs just printing warnings)"
    ),
    ...
    Checks all critical config (database, LLM, health server, circuit breaker)
    and reports errors and warnings. Use --fail-fast to exit with error code
    if validation fails.
    ...
    result = validate_startup_config(fail_fast=False)  # Don't exit here, we handle it
    summary = get_config_summary()
    
    if json_output:
        # Convert ConfigIssue objects to strings for JSON serialization
        output = {
        ...
        if not result.errors and not result.warnings:
            ...
            typer.echo(f"✓ Configuration valid with {len(result.warnings)} warning(s)")
        else:
            typer.echo(f"\n✗ Configuration has {len(result.errors)} error(s)", err=True)
    
    # Exit with error if fail-fast and errors exist
