...
def _check_connection(conn: "psycopg.Connection") -> None:
    ...
    V4 Enhancement: Pool health check callback.
    V4.1 Fix: Must raise on failure (not return bool) per psycopg_pool semantics.
    
    Uses psycopg_pool.ConnectionPool.check_connection which:
    - Returns None on success
    - Raises exception on failure
    
    See docs/PROD_GAP_CLOSURE_PLAN_KG_DB_STREAMING.md Phase 4.
    
    Args:
        conn: psycopg connection to check
        
    Raises:
        Exception: If connection is not alive
    """
    # Use the built-in check_connection which has proper semantics
    # (raises on failure, handles autocommit correctly)
...
def get_pool_config() -> dict:
    ...
    Environment Variables (H-2 Production Hardening):
        POSTGRES_POOL_MIN: Minimum pool size (default: 2)
        POSTGRES_POOL_MAX: Maximum pool size (default: 20)
        POSTGRES_POOL_TIMEOUT: Connection acquisition timeout in seconds (default: 5.0)
        POSTGRES_POOL_MAX_LIFETIME: Max connection lifetime in seconds (default: 3600)
        POSTGRES_POOL_MAX_IDLE: Max idle time before recycling in seconds (default: 300)
    
    Returns:
        Dictionary with validated pool configuration.
    """
    import os
    
    def parse_int_env(name: str, default: int, min_val: int = 1) -> int:
        """Parse integer env var with validation."""
        ...
        try:
            val = int(raw)
            if val < min_val:
                logger.warning(f"{name}={raw} is below minimum ({min_val}), using {min_val}")
                return min_val
            return val
        except ValueError:
    ...
    def parse_float_env(name: str, default: float, min_val: float = 0.0) -> float:
        """Parse float env var with validation."""
        raw = os.environ.get(name)
        if raw is None:
            return default
        try:
    ...
    min_size = parse_int_env("POSTGRES_POOL_MIN", 2, min_val=1)
    max_size = parse_int_env("POSTGRES_POOL_MAX", 20, min_val=1)
    
    # Ensure min <= max
    if min_size > max_size:
        logger.warning(f"POSTGRES_POOL_MIN ({min_size}) > POSTGRES_POOL_MAX ({max_size}), setting min=max")
        min_size = max_size
    
    return {
        "min_size": min_size,
        "max_size": max_size,
        "timeout": parse_float_env("POSTGRES_POOL_TIMEOUT", 5.0, min_val=0.1),
        "max_lifetime": parse_float_env("POSTGRES_POOL_MAX_LIFETIME", 3600.0, min_val=60.0),
        "max_idle": parse_float_env("POSTGRES_POOL_MAX_IDLE", 300.0, min_val=10.0),
    }


# Track if we've logged the config (log once on first pool creation)
...
def get_pool() -> "ConnectionPool":
    ...
    Pool Configuration (H-2 Production Hardening):
        Pool size, timeouts, and connection lifecycle can be configured via
        environment variables. See get_pool_config() for available options.
        Configuration is logged once on first pool creation.
    ...
    V4 Enhancement: Added max_lifetime for connection recycling and
    check callback for connection validation.
    See docs/PROD_GAP_CLOSURE_PLAN_KG_DB_STREAMING.md Phase 4.
    """
    global _pool, _pool_config_logged
    ...
    if _pool is None:
        ...
        # Get validated pool configuration from environment
        ...
        _pool = ConnectionPool(
            ...
            check=_check_connection,  # V4: Validate connection before return
            # No row_factory - use default tuple rows for consistency with SQLite
        )
        
        # Log effective config once on first pool creation (H-2)
        if not _pool_config_logged:
            logger.info(
                f"Postgres connection pool initialized: "
                f"min_size={pool_config['min_size']}, max_size={pool_config['max_size']}, "
                f"timeout={pool_config['timeout']}s, max_lifetime={pool_config['max_lifetime']}s, "
                f"max_idle={pool_config['max_idle']}s"
            )
...
def close_pool() -> None:
    ...
    if _pool is not None:
        _pool.close()
        _pool = None
        _pool_config_logged = False  # Reset so config is logged on next pool creation
...
def get_connection_with_retry(
    ...
    """
    Get a connection from the pool with automatic retry on transient failures.
    
    P0.3 Fix: Adds resilience for connection lifecycle issues like:
    - Connection reset by peer
    - Pool exhaustion
    - Network timeouts
    
    Uses exponential backoff to avoid thundering herd on recovery.
        ...
        try:
            ...
            with pool.connection() as conn:
                ...
                try:
                    ...
                    logger.warning(f"[postgres] Connection health check failed: {e}")
                    raise
            ...
            if not is_retryable or attempt == max_attempts:
                logger.warning(
                    f"[postgres] Connection failed after {attempt} attempt(s): {e}"
                )
                raise
            
            delay = backoff_base * (2 ** (attempt - 1))
            logger.info(
                f"[postgres] Connection attempt {attempt}/{max_attempts} failed: {e}. "
...
-- P0.1 Fix: Added repo_root column and composite unique (repo_root, uri)
-- This allows the same URI to be used in different repo contexts
-- Content dedup is by (source_system_id, sha256)
...
-- Unique constraint: one feedback per run + langsmith_feedback_id combo (for LangSmith feedback)
...
-- Unique constraint: one implicit signal per run + template + type + source combo
-- This prevents duplicate implicit signals on retry (idempotency)
...
def _apply_full_schema(conn) -> None:
    ...
    DEPRECATED: This function is retained for backward compatibility and testing.
    Production code should use migrations as the single source of truth.
    
    The inline DDL constants (SPEC_SILVER_DDL, etc.) serve as documentation
    of the schema but are NOT executed during normal init_all_schemas().
    All schema changes go through migrations/ directory.
    
    See docs/PROD_HARDENING_PLAN_V2.md Item A for rationale.
    """
    with conn.cursor() as cur:
...
def _run_migrations(conn) -> None:
    ...
    This is the SINGLE source of truth for schema evolution. The baseline
    migration (001_baseline_v1.sql) creates all schemas/tables from scratch.
    Future migrations add incremental changes.
    
    Per docs/PROD_HARDENING_PLAN_V2.md Item A: migrations are canonical,
    inline DDL is deprecated.
    
    V31-004: Optionally sync checksums to silence cosmetic mismatch warnings.
    Set BEADS_SYNC_MIGRATION_CHECKSUMS=1 to enable automatic checksum sync.
    """
    import os
    try:
        from .migrations import run_pending_migrations
        # V31-004: Enable checksum sync via env var (default: False for safety)
        sync_checksums = os.environ.get("BEADS_SYNC_MIGRATION_CHECKSUMS", "").lower() in ("1", "true", "yes")
        applied, skipped = run_pending_migrations(conn, sync_checksums=sync_checksums)
        if applied > 0:
            logger.info(f"Applied {applied} database migration(s)")
    except Exception as e:
        # Log but don't fail - migrations are optional enhancement
...
def init_all_schemas(conn=None) -> None:
    ...
    Schema Initialization Strategy (per PROD_HARDENING_PLAN_V2.md Item A):
    - Migrations are the SINGLE source of truth
    - 001_baseline_v1.sql creates all schemas: spec_silver, integration_gold, repo_meta, kg
    - Inline DDL (_apply_full_schema) is DEPRECATED and NOT executed
    - Future schema changes MUST go through migrations/ directory
    
    Safe to call multiple times (migrations are idempotent with version tracking).
    
    Also seeds STANDARD_PATTERNS into kg.nodes after schema init.
    """
    if conn is None:
        with get_connection() as managed_conn:
            # Migrations are canonical - they handle all schema creation
            _run_migrations(managed_conn)
    else:
...
def init_schema_legacy(conn=None) -> None:
    """
    Initialize schema using DEPRECATED inline DDL.
    
    WARNING: This function is ONLY for tests that need to bypass migrations.
    Production code MUST use init_all_schemas() which uses migrations.
    
    This exists for:
    - Tests that need deterministic DDL without migration file dependencies
    - Migration development to compare inline vs migration DDL
    ...
    logger.warning("Schema initialized via DEPRECATED inline DDL (use init_all_schemas for production)")
