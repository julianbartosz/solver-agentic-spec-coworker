...
# =============================================================================
# Tracks when ConnectionWrapper is garbage collected without being closed.
# This indicates a connection leak in calling code.
# =============================================================================
...
def reset_connection_leak_count() -> None:
    """Reset the connection leak count (for testing)."""
    global _connection_leak_count
    with _connection_leak_lock:
        _connection_leak_count = 0
...
class ConnectionWrapper:
    ...
    def __del__(self):
        ...
        B-008: Increment leak counter for observability.
        """
        global _unclosed_connection_warned
        
        if not self._closed:
            # B-008: Track connection leaks for metrics
...
def _warn_sqlite_deprecation() -> None:
    """
    Emit a deprecation warning if SQLite is used outside of test mode.
    
    Per V2 Implementation Plan Section 6.2:
    - USE_SQLITE is deprecated for production use
    - Only suppress warning in test mode
...
def get_sqlite_connection() -> sqlite3.Connection:
    ...
    """
    settings = get_settings()
    sqlite_path = Path(settings.database.sqlite_path) if getattr(settings, "database", None) else DB_PATH
...
def get_connection() -> ConnectionWrapper:
    ...
    if engine == "postgres":
        ...
        # P0.3 Fix: Add retry logic for transient connection failures
        try:
            from .postgres import get_pool, _check_connection
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                # P0.3: Verify connection health before returning
                try:
                    _check_connection(raw_conn)
                except Exception as check_err:
                    # Connection is stale, return it and try again
                    try:
                        pool.putconn(raw_conn)
                    except Exception:
                        pass  # Ignore putconn errors
                    raise check_err
                ...
                is_retryable = any(msg in error_str for msg in [
                    "connection", "pool", "timeout", "reset", "closed", "broken"
                ])
                ...
                logger.info(
                    f"[db] Postgres connection attempt {attempt}/{max_attempts} failed: {e}. "
                    f"Retrying in {delay:.2f}s"
                )
                time.sleep(delay)
        
        raise RuntimeError(
            f"Failed to connect to Postgres database after {max_attempts} attempts: {last_exception}\n"
...
def transaction(isolation_level: str = "READ COMMITTED"):
    ...
    V4 Enhancement: Provides explicit transaction semantics.
    See docs/PROD_GAP_CLOSURE_PLAN_KG_DB_STREAMING.md Phase 4.
    
    This is useful when you need a specific isolation level for
    aggregation queries or when multiple operations must see
    consistent data.
    
    Args:
        isolation_level: One of 'READ COMMITTED', 'REPEATABLE READ', 'SERIALIZABLE'
                         Default is READ COMMITTED (Postgres default).
    
    Usage:
        with db.transaction("REPEATABLE READ") as conn:
            # All operations in this block see consistent snapshot
            cur = conn.cursor()
            cur.execute("SELECT SUM(value) FROM table")
            total = cur.fetchone()[0]
            cur.execute("UPDATE summary SET total = %s", (total,))
            # Commits automatically on successful exit
    
    Yields:
        ConnectionWrapper: Database connection for use in transaction
        
    Raises:
        Exception: Re-raises any exception after rolling back
        
    Note:
        For Postgres, uses psycopg3's IsolationLevel enum via connection.transaction()
        context manager which properly sets isolation BEFORE the transaction begins.
        For SQLite, isolation is always SERIALIZABLE (SQLite default).
    """
    engine = get_engine_type()
    
    if engine == "postgres":
        # Use psycopg3's proper transaction API
        ...
        iso_level = level_map.get(isolation_level.upper(), IsolationLevel.READ_COMMITTED)
        
        # Get raw psycopg connection from pool
        from .postgres import get_pool
        pool = get_pool()
        raw_conn = pool.getconn()
        
        try:
            # Use psycopg3's transaction() context manager which sets isolation
            # level BEFORE the transaction begins (not via SET TRANSACTION)
            with raw_conn.transaction(isolation_level=iso_level):
                # Wrap for consistent API
                wrapper = ConnectionWrapper(raw_conn, pool=pool, engine="postgres")
                yield wrapper
            # Transaction commits automatically on successful exit from with block
        except Exception:
        ...
        # SQLite: Use simple connection (isolation is always SERIALIZABLE)
        conn = get_connection()
        try:
            yield conn
            conn.commit()
...
def _init_sqlite_schema() -> None:
    ...
    # Create unique index for deduplication (LangSmith feedback)
    cur.execute("""
    ...
    # Create unique index for implicit signal deduplication (idempotency on retry)
    cur.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS kg_feedback_implicit_dedup_idx 
        ON kg_feedback_records(run_id, template_key, feedback_type, source)
