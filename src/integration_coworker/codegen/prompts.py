...
- Validate that api_key is not empty and payload is not empty/None
- DO NOT validate specific fields inside the payload - just check payload is not empty
- Transform and return the API response
- Handle API errors via the client

CRITICAL: Keep validation simple!
- ONLY check: api_key is not empty, payload is not None/empty
- DO NOT check for specific keys like "AlphaSenderId", "line_items", etc.
- Let the API return errors for missing/invalid fields"""
...
CRITICAL TEST STRUCTURE RULES (Bug #91 - MUST follow):
- DO NOT add extra test methods beyond what's in the skeleton
- The skeleton provides exactly 6 test methods - DO NOT add more
- DO NOT test for specific payload fields (service_sid, owner, alpha_sender, etc.)
- ONLY test for empty api_key and empty payload - not specific field validation
