...
try:
    ...
    logger.debug("tree-sitter not installed, using fallback validation")
...
def _load_tree_sitter_languages():
    ...
    for dist in pkg_resources.working_set:
        if dist.key.startswith("tree-sitter-") and dist.key != "tree-sitter":
            lang_name = dist.key.replace("tree-sitter-", "").replace("-", "_")
