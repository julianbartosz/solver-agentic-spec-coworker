...
class PolicyCodeSnippet:
    ...
    V39-004: Added module_level_code field for definitions that must appear
    at the module level (like exception classes), not inside a class body.
    """
    ...
    module_level_code: str = ""  # V39-004: Code that must appear at module level (e.g., exception classes)
...
''',
    ...
    def _refresh_oauth_token(self) -> None:
        ...
        response = httpx.post(
            ...
            timeout=30.0,  # Explicit timeout for production safety
        )
...
def get_retry_template(config: Dict[str, Any]) -> PolicyCodeSnippet:
    ...
    V39-004: Fixed RetryableError class definition. Now properly defined at
    module level via module_level_code instead of in wrapper_code (which gets
    indented inside a class body, breaking the definition).
    """
    ...
    return PolicyCodeSnippet(
        ...
        # V39-004: Define RetryableError at module level
        module_level_code='''
class RetryableError(Exception):
    """
    Exception raised for errors that should trigger a retry.
    
    Used to signal transient failures (rate limits, server errors) that
    may succeed on subsequent attempts with exponential backoff.
    """
    ...
    def __str__(self) -> str:
        if self.status_code:
            return f"RetryableError({self.status_code}): {self.message}"
...
''',
        ...
        post_request_code="",
        wrapper_code=f'''
    def _with_retry(self, operation: Callable[[], _RT]) -> _RT:
        """Execute operation with {'exponential' if backoff_type == 'exponential' else 'linear'} backoff retry."""
        last_exception: Exception | None = None
        # BUG-TYPE-001 FIX: Use float for delay_ms to avoid type errors with jitter multiplication
        delay_ms: float = float(self._initial_delay_ms)
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                if attempt < self._max_retries - 1:
                    ...
                    jitter = random.uniform(0.8, 1.2)  # nosec B311 - jitter for backoff, not crypto
                    sleep_time = min(delay_ms * jitter, float(self._max_delay_ms)) / 1000.0
                    time.sleep(sleep_time)
                    {'delay_ms *= 2.0' if backoff_type == 'exponential' else 'delay_ms += float(self._initial_delay_ms)'}
...
def build_policy_code(
    ...
    V39-004: Added module_level_code support for definitions that must appear
    at module level (like exception classes).
    ...
    Returns:
        ...
        - module_level_code: Combined module-level definitions (V39-004)
    ...
    all_module_level: List[str] = []  # V39-004
    all_setup: List[str] = []
    ...
    for policy in policies:
        # Skip malformed policies (e.g., strings from bad LLM responses)
        ...
        # Ensure config is a dict, not a string from malformed LLM response
        if not isinstance(config, dict):
            config = {}
        snippet = template_getter(config)

        all_imports.extend(snippet.imports)
        # V39-004: Collect module-level code
        if snippet.module_level_code.strip():
            all_module_level.append(snippet.module_level_code)
...
def inject_policies_into_client_code(
    ...
    V39-004: Now supports module_level_code for definitions that must
    appear at module level (like exception classes).
    
    This modifies the client code to include:
    - Policy-related imports
    - Module-level definitions (V39-004)
    ...
    imports_added = False
    module_level_added = False
    in_init = False
    init_done = False
    last_import_index = -1
    first_class_index = -1

    # First pass: find last import line and first class definition
    for i, line in enumerate(lines):
        if line.startswith("import ") or line.startswith("from "):
            last_import_index = i
        if line.startswith("class ") and first_class_index == -1:
            first_class_index = i

    for i, line in enumerate(lines):
        # Add imports and module-level code after last import line
        if not imports_added and i == last_import_index:
            result_lines.append(line)
            
            if policy_code["imports"]:
                result_lines.append("")
                result_lines.append("# Policy imports")
                result_lines.append(policy_code["imports"])
            
            # V39-004: Add module-level code (like exception classes) after imports
            if policy_code["module_level_code"]:
                result_lines.append("")
                result_lines.append("# Policy module-level definitions")
                result_lines.append(policy_code["module_level_code"])
            
            imports_added = True
            module_level_added = True
            continue
