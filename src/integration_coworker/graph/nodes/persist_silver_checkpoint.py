...
def persist_silver_checkpoint(state: WorkflowState) -> WorkflowState:
    ...
    V28-002 Fix: All database operations are now properly inside the connection
    context manager to prevent "connection is closed" errors.
    ...
    # Variables that need to be accessible after the with block for state updates
    ...
    try:
        ...
        # V28-002 Fix: Use context manager AROUND ALL database operations
        # The V27-003 fix incorrectly closed the connection before using the cursor.
        # All cursor operations must be inside the with block.
        with db.get_connection() as conn:
            ...
            for spec_doc in state.spec_documents:
                ...
                # The row might exist due to either constraint, so check both
            ...
            # V23-CACHE: Use state.primary_spec_document_id (survives state_gc) as primary fallback
            # This fixes cache hydration bug where spec_documents is cleared by state_gc
            primary_spec_document_id = state.primary_spec_document_id
            if not primary_spec_document_id and state.spec_documents:
                primary_spec_document_id = state.spec_documents[0].id
            if not primary_spec_document_id:
                logger.warning("V23-CACHE: No primary_spec_document_id available for persistence")
            ...
            # 6. Insert Endpoints
            # V23-CACHE: Pre-validate all endpoints have valid spec_document_id
            endpoints_without_fk = [e for e in state.endpoints if not e.spec_document_id and not primary_spec_document_id]
            if endpoints_without_fk:
                logger.error(f"V23-CACHE BUG: {len(endpoints_without_fk)} endpoints have no spec_document_id and no fallback available")
                for e in endpoints_without_fk[:5]:  # Log first 5 for debugging
                    logger.error(f"  - {e.method} {e.path}")
            ...
            for endpoint in state.endpoints:
                ...
                # V23-CACHE: Final validation - if still None, log error
                if not doc_id:
                    logger.error(f"V23-CACHE: Endpoint {endpoint.method} {endpoint.path} has no spec_document_id, skipping")
                    continue
            ...
            if chunks_already_streamed:
                ...
                # V22-007: Use bulk insert for performance
                ...
                chunk_rows = []
                chunk_doc_ids = {}  # Track resolved doc_id per chunk_index
                ...
                for chunk in state.spec_chunk_embeddings:
                    ...
                    chunk_rows.append((doc_id, chunk.chunk_index, content, embedding_json))
                    chunk_doc_ids[chunk.chunk_index] = doc_id
                
                # V22-007: Bulk insert all chunks at once
                if chunk_rows:
                    logger.debug(f"Bulk inserting {len(chunk_rows)} spec_chunks")
                    write_rows_insert(
                        conn,
                        "spec_chunks",
                    ...
                    # Backfill IDs in a single query per doc_id
                    for chunk in state.spec_chunk_embeddings:
                        doc_id = chunk_doc_ids.get(chunk.chunk_index)
        ...
        # Update persisted_ids (outside with block - connection is closed but we have the values)
        ...
        # Bug #101 Fix: Clear large content fields after persisting to DB
        # This reduces LangGraph checkpoint size from 100MB+ to <10MB
        # The data is now in the database and can be reloaded if needed
        for spec_doc in state.spec_documents:
            if hasattr(spec_doc, 'content'):
                spec_doc.content = None  # Clear raw content
        for section in state.spec_sections:
            if hasattr(section, 'content'):
                section.content = None  # Clear section content
        # Clear embedding vectors (large float arrays)
        for chunk in state.spec_chunk_embeddings:
            if hasattr(chunk, 'embedding'):
                chunk.embedding = None
        logger.debug("Cleared large content fields from spec_documents, spec_sections, spec_chunk_embeddings")
