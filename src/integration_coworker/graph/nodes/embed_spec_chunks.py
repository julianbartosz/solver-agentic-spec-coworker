...
def _is_auth_error(error: Exception) -> bool:
    """Check if an exception is an auth/credentials error (fail-fast, never retry)."""
    # Check for our typed exception first
...
def _embed_with_retry(
    ...
    Production Readiness v4: Auth errors fail-fast, never retried.
    ...
    Raises:
        LLMAuthError: Immediately on auth/credentials errors (never retried)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if _is_auth_error(e):
                logger.error(f"Auth error in embedding call, not retrying: {e}")
                # Re-raise as typed exception if not already
...
def _batch_embed(client: EmbeddingsProtocol, texts: List[str], model: str) -> List[Optional[List[float]]]:
    ...
    for batch_num, batch in enumerate(batches):
        # V22-011: Check memory ceiling inside loop - don't wait for node boundary
...
def _embed_spec_chunks_legacy(state: WorkflowState) -> WorkflowState:
    ...
    for idx, chunk in enumerate(state.doc_chunks):
        ...
        # V24-002: Check shutdown periodically during embedding loop
        if idx > 0 and idx % 100 == 0:
            try:
                shutdown_check_point(f"embed_spec_chunks: embedding chunk {idx}/{total_chunks}")
            except ShutdownInterruptError:
                logger.warning(f"V24-002: Shutdown requested after embedding {idx} chunks")
                state.warnings.append(f"Embedding interrupted after {idx}/{total_chunks} chunks")
                break
...
def _embed_spec_chunks_streaming(state: WorkflowState) -> WorkflowState:
    ...
    V4 Enhancement: Added progress tracking via run_id for resume support.
    ...
    for spec_doc_id in spec_doc_ids:
        ...
        for chunk_id, chunk_index, content in iter_chunks_for_embedding(spec_doc_id):
            # V22-011: Check memory ceiling inside loop - critical for large specs
    ...
    # V4: Clear progress on successful completion
    if run_id and total_embedded > 0 and failed_count == 0:
        clear_streaming_progress(run_id, "embeddings")
        logger.debug(f"Cleared embedding progress for run_id={run_id}")
    ...
    # V22-MEM: Use state_gc for cleanup after embeddings are stored in DB
    # Embeddings are in DB, no need to keep them in state
    try:
        from integration_coworker.graph.state_gc import cleanup_after_embedding
        cleanup_after_embedding(state)
    except Exception as e:
        logger.debug(f"state_gc cleanup_after_embedding failed (non-fatal): {e}")
...
def _process_embedding_batch(
    ...
    Bug #58 fix: Uses _embed_with_retry for rate limit handling.
    V4 Enhancement: Passes run_id for progress tracking/resume.
    ...
    Args:
        ...
        total_items: Total items being processed (for progress %)
