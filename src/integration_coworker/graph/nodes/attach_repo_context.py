...
def attach_repo_context(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        # Bug #101 Fix: Extract needed data, then clear snapshot to prevent checkpoint bloat
        # The snapshot can be 100MB+ for large repos, and we only need the markdown context
        state.repo_markdown_context = snapshot.full_markdown if snapshot else None
        
        # Keep minimal snapshot info for debugging (tree structure only)
        if snapshot:
            # Keep tree_markdown for debugging/display (typically <50KB)
            # but clear the files dict which contains actual file contents
            snapshot.files = {}  # Clear file contents to reduce memory
...
def _detect_python_framework(repo_path: Path, io: Optional[RepoIO] = None) -> str:
    """
    ...
    P1: Uses RepoIO if available for policy-enforced reads.
    """
        ...
        try:
            if io:
                if io.exists(rel_path):
                    return io.read_text(rel_path).lower()
            else:
                full_path = repo_path / rel_path
                if full_path.exists():
                    return full_path.read_text().lower()
        except Exception:
            pass
        return ""
    
    dep_text += _safe_read("pyproject.toml")
    dep_text += _safe_read("requirements.txt")
    dep_text += _safe_read("setup.py")
...
def _infer_profile_from_conventions(repo_path: Path):
    ...
    if has_pyproject or has_requirements or has_setup_py:
        ...
        framework = _detect_python_framework(repo_path, io)
...
def _get_profile_config_first(
    ...
    # Align defaults with GENERIC_PYTHON_PROFILE to keep imports under src/
