...
def align_task_with_kg(state: WorkflowState) -> WorkflowState:
    ...
    # ---------------------------------------------------------------------------
    # V4 Enhancement: Spec-first entity/endpoint extraction for KG scoring
    # Addresses scoring collapse when entities/endpoints not provided by user
    # See docs/PROD_GAP_CLOSURE_PLAN_KG_DB_STREAMING.md Phase 1
    # ---------------------------------------------------------------------------
    if state.openapi_spec and task_description:
        try:
            ...
            if extracted_entities:
                ...
                logger.debug(
                    "align_task_with_kg: Extracted %d entities from spec: %s",
                    len(extracted_entities), extracted_entities[:5]
                )
            if extracted_endpoints:
                known_endpoints = list(set((known_endpoints or []) + extracted_endpoints))
                logger.debug(
                    "align_task_with_kg: Extracted %d endpoints from spec: %s",
                    len(extracted_endpoints), extracted_endpoints[:5]
                )
        except Exception as e:
            logger.warning("align_task_with_kg: Entity extraction failed: %s", e)
