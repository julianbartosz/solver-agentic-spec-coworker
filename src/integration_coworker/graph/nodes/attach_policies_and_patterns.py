...
def _extract_security_schemes(state: WorkflowState) -> Dict[str, Any]:
    ...
    # Fallback to state.openapi_spec if plan didn't populate openapi_specs (e.g., direct state injection)
    if not specs and state.openapi_spec:
        specs = [state.openapi_spec]

    # Last-resort fallback: parse first spec_ref on demand to extract securitySchemes
    if not specs and state.spec_refs:
        try:
            import yaml
            from pathlib import Path

            ref_path = Path(state.spec_refs[0])
            if ref_path.exists():
            ...
            logger.debug(f"Failed to parse spec_refs fallback for securitySchemes: {e}")
    ...
    # If no explicit securitySchemes, attempt to heuristically detect
    # common API key patterns in operation parameters (some specs inline params)
    if not schemes:
        ...
        for path_item in paths.values():
            for operation in (path_item or {}).values():
                if not isinstance(operation, dict):
                    continue
                # Check parameters for api_key-like names or header names
                for param in operation.get("parameters", []):
                    try:
                        pname = param.get("name", "").lower()
                        pin = param.get("in", "").lower()
                    except Exception:
                        continue
                    if pname in ("api_key", "apikey", "x-api-key") or pname.startswith("x-") and "api" in pname:
                        # Construct a minimal apiKey scheme
                        schemes = {
                            "inferred_api_key": {
                                "type": "apiKey",
                                "in": pin or "header",
                                "name": param.get("name", "api_key"),
                            }
                        }
                        return schemes
...
async def _augment_with_llm_analysis(
    ...
    V26-PERF: Optimized for large specs:
    - Early return if spec policies are sufficient
    - Limited prose collection scope
    ...
    # V26-PERF: Early return if we already have all 4 main policy types from spec
    # This avoids expensive LLM calls when structured spec data is complete
    required_types = {"auth", "rate_limit", "retry", "idempotency"}
    if required_types.issubset(policies.keys()):
        logger.info("[V26-PERF] Spec policies complete, skipping LLM augmentation")
        return policies
    
    # V26-PERF: Reduce scope - only collect prose if we're missing policies
    missing_types = required_types - policies.keys()
    logger.debug(f"[V26-PERF] Missing policy types: {missing_types}, collecting prose")
    ...
    if specs:
        ...
        if info.get("description"):
            # V26-PERF: Limit description length
            prose_sections.append(f"API Description:\n{info['description'][:1000]}")
        
        # V26-PERF: Collect only first 3 endpoints (reduced from 5)
        paths = spec.get("paths", {})
        for path, path_item in list(paths.items())[:3]:
            for method, operation in path_item.items():
                if isinstance(operation, dict):
                    desc = operation.get("description", "")
                    summary = operation.get("summary", "")
                    if desc or summary:
                        # V26-PERF: Reduced description length
    ...
    # V26-PERF: Limit total prose to avoid token overflow
    prose_text = "\n\n".join(prose_sections[:5])  # Reduced from 10
...
"""
    ...
    try:
        ...
        # V26-PERF: Add timeout protection for LLM calls
        try:
            response = await asyncio.wait_for(
                client.complete_async(prompt),
                timeout=30.0  # 30 second timeout
            )
        except asyncio.TimeoutError:
            logger.warning("[V26-PERF] LLM augmentation timed out after 30s, using spec-only policies")
            return policies
