...
V4: Added retry wrapper to compute_embedding for production resilience.
...
# Retry configuration (aligned with embed_spec_chunks.py)
MAX_RETRIES = 3
INITIAL_BACKOFF = 2.0
MAX_BACKOFF = 60.0
...
def compute_embedding(text: str, max_retries: int = MAX_RETRIES) -> List[float]:
    """
    Compute embedding for query text with exponential backoff + jitter.
    
    V4 Enhancement: Added retry logic for production resilience.
    See docs/PROD_GAP_CLOSURE_PLAN_KG_DB_STREAMING.md Phase 2.
    ...
    Raises:
        LLMAuthError: Immediately on auth errors (never retried)
    """
    ...
    try:
        from integration_coworker.graph.nodes.embed_spec_chunks import (
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if _is_auth_error(e):
                logger.error(f"Auth error in compute_embedding, not retrying: {e}")
                raise LLMAuthError(str(e)) from e
            
            # Only retry on rate limit / transient errors
            if not _is_rate_limit_error(e) or attempt == max_retries:
                logger.warning(
                    f"compute_embedding failed after {attempt + 1} attempts: {e}"
                )
            ...
            logger.warning(
                f"compute_embedding rate limit hit (attempt {attempt + 1}/{max_retries + 1}), "
...
def _search_spec_chunks_python(
    ...
    V29-P01 Fix: Uses context manager for db connection to prevent leaks.
    """
    try:
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                for row in rows:
                    ...
                    graph_signal = 0.5 if provider_code else 0.0
                    graph_weight = 0.4
                    semantic_weight = 0.6
                    combined_score = (
    ...
    try:
        with db.get_connection() as conn:
            ...
            for row in rows:
                ...
                # V2: Graph score based on provider match.
                # Keep a small baseline for reporting but do not contribute to combined
                # score when no provider is supplied to avoid inflating matches.
                graph_score = 0.5 if provider_code else 0.3
                graph_signal = 0.5 if provider_code else 0.0

                # V2: Combined score (graph-weighted higher for provider matches)
                graph_weight = 0.4
                semantic_weight = 0.4
                combined_score = (
                    graph_signal * graph_weight +
