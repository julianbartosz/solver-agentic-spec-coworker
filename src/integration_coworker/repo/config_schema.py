...
def load_config(config_path: Path) -> Optional[IntegrationCoworkerConfig]:
    ...
    # This ensures consistency even if an existing config has 'workflows'
    if content.get("layout") and content["layout"].get("flows_dir"):
        flows_dir = content["layout"]["flows_dir"]
        if "workflow" in flows_dir.lower():
            original = flows_dir
            flows_dir = flows_dir.replace("workflows", "flows").replace("workflow", "flows")
            content["layout"]["flows_dir"] = flows_dir
            logger.info(f"[V41-004] Normalized flows_dir in loaded config: '{original}' -> '{flows_dir}'")
