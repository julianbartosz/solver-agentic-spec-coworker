...
Example:
    >>> from integration_coworker_runtime import ExponentialRetry, IntegrationClient
    >>> 
...
import random
import time
from typing import Callable, Optional, Protocol, Set, TypeVar, runtime_checkable
...
class NoRetry:
    ...
    def execute(self, func: Callable[[], T]) -> T:
        """Execute function once without retry."""
        return func()
...
class ExponentialRetry:
    ...
    Example:
        >>> retry = ExponentialRetry(max_attempts=3, base_delay=1.0)
        >>> result = retry.execute(lambda: make_api_call())
    """
