...
- [ ] Add `FeedbackRecord`, `FeedbackType` to domain/models.py
- [ ] Add `kg.feedback_records`, `kg.confidence_history` to DDL
- [ ] Create `src/integration_coworker/feedback/` module skeleton

### Phase 2: LangSmith Sync (Day 1-2)
- [ ] Implement `langsmith_sync.py`:
...
- [ ] Add `kg.confidence_history` logging
- [ ] Update `kg/__init__.py` to use confidence in scoring

### Phase 4: Implicit Signals (Day 2-3)
- [ ] Implement `implicit_signals.py`:
  - `record_compile_result()` - From validate_integration_design
  - `record_lint_result()` - From ruff checks
- [ ] Wire into `persist_run_outcome` node

### Phase 5: CLI & Testing (Day 3)
- [ ] Add `feedback` CLI command for manual feedback
- [ ] Add `kg-confidence` CLI command
- [ ] Write tests for:
