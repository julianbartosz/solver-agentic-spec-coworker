...
MkDocs tests load ``mkdocs.yml`` with ``yaml.safe_load`` so callables are
stored as strings in the config. This plugin imports the callable and
replaces the string so pymdownx.superfences continues to render mermaid
blocks using ``fence_code_format``.
"""
...
class MermaidFenceResolverPlugin(BasePlugin):
    """Resolve string formats in pymdownx.superfences to callables."""

    config_scheme: tuple = ()

    def on_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Replace string ``format`` values with imported callables."""
