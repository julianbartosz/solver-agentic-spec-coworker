...
## Bucket 2 Runtime Logging (Dec 2025 refresh)

- **Entry point:** `src/integration_coworker/cli.py` now configures logging at CLI startup via a Typer callback (lines ~288-300) plus per-command upgrades when `--verbose` is passed. The helper `_setup_logging` is idempotent and preserves higher-verbosity configurations.
- **Defaults:** human-readable logs at WARNING level; dependencies reuse the shared handler.
- **JSON logs:** set `JSON_LOGS=1` to emit structured JSON via `logging_config.StructuredFormatter`. Example:
    - `JSON_LOGS=1 integration-coworker health --json`
- **No duplicate handlers:** repeated CLI invocations within the same process keep a single handler while allowing a DEBUG upgrade when `--verbose` is supplied.

This wiring ensures production entrypoints emit structured logs without relying on test-only helpers.
