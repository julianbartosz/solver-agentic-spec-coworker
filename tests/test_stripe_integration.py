...
class TestStripePaymentIntents:
    ...
    def test_stripe_create_payment_intent_dry_run(self, stripe_spec):
        ...
        # Task slug should be normalized (mock LLM may not always infer "payment")
        # Just verify a task_slug was generated and is non-empty
        assert result.task.task_slug is not None
        assert len(result.task.task_slug) > 0
