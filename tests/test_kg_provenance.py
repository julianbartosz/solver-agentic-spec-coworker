...
class TestPersistFileSpecWithFields:
    ...
    def test_persist_spec_with_guide_fields_adds_derives_edges(self):
        """Guide fields create GUIDE_FIELD nodes and DERIVES_FROM_GUIDE edges."""
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_conn.cursor.return_value = mock_cursor
        # spec, guide doc node, spec->guide edge, field1 node/edge, field2 node/edge, guide_field node, field->guide edge
        mock_cursor.fetchone.side_effect = [
            (100,),  # spec
            (200,),  # guide doc
            (300,),  # spec->guide edge
            (201,), (1,),  # field1 node, has_field
            (202,), (2,),  # field2 node, has_field
            (301,),        # guide_field node
            (3,),          # field->guide edge
        ]
        ...
        fields = [
            FileField(id=1, file_spec_id=1, name="account_number", field_type="string", position=1),
            FileField(id=2, file_spec_id=1, name="amount", field_type="number", position=2),
        ]
        guide_fields = [
            {"name": "account_number", "field_type": "string", "confidence": 0.92},
        ]
        ...
        persist_file_spec_with_fields(
            ...
            file_spec,
            fields,
            guide_uri="guide://example.pdf",
            guide_fields=guide_fields,
            compute_embeddings=False,
        )

        # Find the DERIVES_FROM_GUIDE edge call between guide_field and file_field
        derives_call = None
        for call in mock_cursor.execute.call_args_list:
            ...
            if (
                "kg.edges" in sql
                and params[2] == KGEdgeRelation.DERIVES_FROM_GUIDE.value
                and params[0] == 301  # guide_field_node_id
            ):
                derives_call = call
                break

        assert derives_call, "DERIVES_FROM_GUIDE edge was not written"
        _, edge_params = derives_call[0][0], derives_call[0][1]
        props_json = edge_params[3]
        if isinstance(props_json, str):
            props_json = json.loads(props_json)
        assert "confidence" in props_json
        assert props_json.get("match_method") == "name_match"
        assert props_json.get("guide_uri") == "guide://example.pdf"
...
class TestFindMatchingFileFields:
    """Test matching helper for guide fields."""

    @patch('integration_coworker.kg._compute_embedding')
    @patch('integration_coworker.kg.persist.find_similar_fields')
    @patch('integration_coworker.kg.persist.db.get_engine_type', return_value="postgres")
    def test_find_matching_file_fields_returns_results(self, mock_engine, mock_find, mock_embed):
        mock_embed.return_value = [0.1, 0.2, 0.3]
        mock_find.return_value = [
            (10, "file_field.file_spec.1.col1", 0.88, {"file_spec_key": "file_spec.1.test"}),
        ]

        mock_conn = MagicMock()
        # Force deterministic name lookup to miss so vector path is exercised
        mock_conn.cursor.return_value.fetchone.return_value = None
        results = find_matching_file_fields(
            mock_conn,
            {"name": "Account", "description": "Account identifier", "source_uri": "guide://u"},
            min_similarity=0.5,
            file_spec_key="file_spec.1.test",
        )

        assert len(results) == 1
        assert results[0]["file_field_node_id"] == 10
        assert results[0]["method"] == "vector_similarity"
        mock_find.assert_called_once()

    @patch('integration_coworker.kg.persist.find_similar_fields')
    @patch('integration_coworker.kg.persist.db.get_engine_type', return_value="sqlite")
    def test_find_matching_file_fields_skips_vector_on_sqlite(self, mock_engine, mock_find):
        mock_conn = MagicMock()
        mock_conn.cursor.return_value.fetchone.return_value = None
