...
class TestSetNestedBugFix:
    """Tests for _set_nested bug fix (list intermediate values).
    
    Bug #16: When parsing TOON, if an intermediate key already contains a list
    (e.g., from `constraints.extra=[...]`), setting a nested key under it would
    crash with "'list' object has no attribute 'get'".
    
    These tests ensure robust handling of non-dict intermediate values.
    """
    
    def test_set_nested_basic(self):
        """Test basic nested key setting."""
        from integration_coworker.llm.toon import _set_nested
        d = {}
        _set_nested(d, ["a", "b"], "val")
        assert d == {"a": {"b": "val"}}
    
    def test_set_nested_list_intermediate(self):
        """Test _set_nested doesn't crash when intermediate value is a list.
        
        This reproduces the exact bug scenario:
        1. Parse `constraints.extra=[...]` → creates constraints.extra as list
        2. Try to parse `constraints.extra.something=value` → would crash
        """
        from integration_coworker.llm.toon import _set_nested
        d = {"constraints": {"extra": [1, 2, 3]}}
        # This should NOT raise "'list' object has no attribute 'get'"
        _set_nested(d, ["constraints", "extra", "protocol_type"], "rest")
        # The list should be preserved under _value, and new key added
        assert d["constraints"]["extra"]["_value"] == [1, 2, 3]
        assert d["constraints"]["extra"]["protocol_type"] == "rest"
    
    def test_set_nested_string_intermediate(self):
        """Test _set_nested handles string intermediate value."""
        from integration_coworker.llm.toon import _set_nested
        d = {"config": "some_string"}
        _set_nested(d, ["config", "nested"], "val")
        # The string should be preserved under _value
        assert d["config"]["_value"] == "some_string"
        assert d["config"]["nested"] == "val"
    
    def test_set_nested_int_intermediate(self):
        """Test _set_nested handles int intermediate value."""
        from integration_coworker.llm.toon import _set_nested
        d = {"count": 42}
        _set_nested(d, ["count", "nested"], "val")
        assert d["count"]["_value"] == 42
        assert d["count"]["nested"] == "val"
    
    def test_from_toon_with_conflicting_nested_paths(self):
        """Test full from_toon parsing when paths conflict.
        
        Real-world scenario where LLM returns both a simple value and nested
        values for the same key prefix.
        """
        toon = """constraints.extra=[target1,target2]
constraints.extra.protocol_type=rest
constraints.idempotency_required=true"""
        ...
        assert result["constraints"]["idempotency_required"] is True
        # The extra key should have both the list (as _value) and nested key
        assert result["constraints"]["extra"]["_value"] == ["target1", "target2"]
        assert result["constraints"]["extra"]["protocol_type"] == "rest"
