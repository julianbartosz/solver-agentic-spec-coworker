...
def test_generate_router_block():
    ...
    V40-001: Updated to test new route handler generation pattern instead
    of the broken include_router pattern that tried to access .router on
    flow functions.
    """
    ...
    assert "router.include_router" not in block
    # V40-001: Should create proper route handler
    assert "@router.post" in block
    assert "/stripe/create-checkout-session" in block
...
def test_generate_router_block_with_custom_module():
    ...
    V40-001: Updated to test new route handler pattern.
    V41-003: Updated - src. prefix is stripped for Python imports since
             PYTHONPATH=src is the standard convention.
    """
    ...
    # V41-003: src. prefix is stripped, so expect "integrations.flows" not "src.integrations.flows"
    assert "from integrations.flows.stripe_create_checkout_session import" in block
    # V40-001: Should use route handler, not include_router
    assert "@router.post" in block
    assert "/stripe/create-checkout-session" in block
