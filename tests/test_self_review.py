...
class TestApplyReviewResult:
    ...
    def test_fail_no_fix_development_security_blocks(self):
        """Test that security issues block even in development mode (V39-002).
        
        Security issues (category=SECURITY) are NEVER allowed to pass through,
        regardless of is_production setting. This ensures security vulnerabilities
        are caught early in all environments.
        """
        original = "exec(user_input)"
        result = ReviewResult(
            verdict="fail",
        ...
        assert code == original
        assert success is False  # Security issues always block
        assert error is not None
        assert "SECURITY" in error or "security" in error.lower()
    
    def test_fail_no_fix_development_style_warns(self):
        """Test that non-security issues warn but succeed in development.
        
        Non-security issues (STYLE, LOGIC, PERFORMANCE) are allowed to pass
        with warnings in development mode to enable rapid iteration.
        """
        ...
        assert code == original
        assert success is True  # Non-security issues allowed in development
        assert error is None or "[WARNINGS]" in str(error)
...
class TestReviewAndRepair:
    ...
    @pytest.mark.asyncio
    async def test_unfixable_security_development_blocks(self):
        """Test unfixable SECURITY issue blocks even in development (V39-002).
        
        Security issues are never allowed to pass through with graceful 
        degradation - they must be fixed or blocked, regardless of mode.
        """
        mock_llm = MagicMock()
        mock_structured = AsyncMock()
        mock_structured.ainvoke.return_value = ReviewResult(
    ...
    @pytest.mark.asyncio
    async def test_unfixable_style_development_warns(self):
        """Test unfixable STYLE issue warns but continues in development.
        
        Non-security issues use graceful degradation in development mode -
        code proceeds with warnings to enable rapid iteration.
        """
        mock_llm = MagicMock()
        mock_structured = AsyncMock()
        mock_structured.ainvoke.return_value = ReviewResult(
            ...
            issues=[ReviewIssue(
                category=IssueCategory.STYLE,
                severity=IssueSeverity.WARNING,
                explanation="Inconsistent formatting - cannot auto-fix",
            )],
        )
        ...
        with patch(
            ...
            code, success, error = await review_and_repair(
                code='def foo():pass  # no spaces',
                artifact_type="client",
                file_path="src/style_issue.py",
                llm=mock_llm,
                is_production=False,  # Development mode
            )
