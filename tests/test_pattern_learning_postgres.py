...
# Mark all tests in this module as requiring Postgres (skipped when SQLite/offline)
pytestmark = [
    pytest.mark.postgres,
    pytest.mark.skipif(
        os.getenv("USE_SQLITE", "").lower() == "true"
        or os.getenv("VALIDATION_PROFILE", "").lower() == "offline",
        reason="Postgres-only pattern learning tests are skipped in SQLite/offline mode",
    ),
]
