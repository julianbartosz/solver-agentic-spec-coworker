...
class TestArchetypeLoading:
    ...
    def test_archetype_caching(self):
        """Test that archetypes are cached after first load (H-4 update)."""
        archetype1 = load_archetype("understand_task")
        archetype2 = load_archetype("understand_task")
        
        # H-4: Now uses @lru_cache internally. The base config is cached,
        # but env overrides are applied fresh each time, creating new dict objects.
...
class TestArchetypeMutationSafety:
    """Tests for mutation safety of archetype returns (production hardening)."""
    
    def test_mutation_does_not_affect_other_callers(self):
        """
        PRODUCTION PROOF: Mutating a returned archetype does not affect other calls.
        
        This is critical for production safety - multiple callers may modify the
        archetype dict, and changes must not leak between callers.
        """
        # Get first copy
        archetype1 = load_archetype("understand_task")
        original_provider = archetype1.get("model", {}).get("provider")
        
        # Mutate it
        archetype1["model"]["provider"] = "MUTATED_PROVIDER"
        archetype1["MUTATED_KEY"] = "MUTATED_VALUE"
        
        # Get second copy
        archetype2 = load_archetype("understand_task")
        
        # PROOF: Second copy should NOT see the mutation
        assert archetype2.get("model", {}).get("provider") == original_provider
        assert "MUTATED_KEY" not in archetype2
    
    def test_nested_mutation_does_not_affect_other_callers(self):
        """
        PRODUCTION PROOF: Deep mutations do not leak between callers.
        """
        # Get first copy
        archetype1 = load_archetype("understand_task")
        
        # Deep mutation
        if "model" in archetype1:
            archetype1["model"]["deeply_nested"] = {"a": 1, "b": 2}
        
        # Get second copy
        archetype2 = load_archetype("understand_task")
        
        # PROOF: Second copy should NOT see the deep mutation
        assert "deeply_nested" not in archetype2.get("model", {})
    
    def test_list_mutation_does_not_affect_other_callers(self):
        """
        PRODUCTION PROOF: Mutating lists in archetype does not leak.
        """
        # Get first copy
        archetype1 = load_archetype("understand_task")
        
        # Find a list to mutate (or add one)
        if "retrieval" not in archetype1:
            archetype1["retrieval"] = {}
        archetype1["retrieval"]["sources"] = ["mutated_source"]
        
        # Get second copy  
        archetype2 = load_archetype("understand_task")
        
        # PROOF: Second copy should NOT see the list mutation
        sources2 = archetype2.get("retrieval", {}).get("sources", [])
        assert "mutated_source" not in sources2
