...
class TestBackwardCompatibility:
    """Tests ensuring older LLM responses still parse correctly."""

    @patch("integration_coworker.graph.nodes.understand_task.call_llm_async_for_node")
    @pytest.mark.asyncio
    async def test_response_without_protocol_type_defaults_to_rest(self, mock_llm):
        """
        LLM responses without protocol_type should default to REST.
        
        This ensures backward compatibility with older prompts/responses
        before the file integration feature was added.
        """
        # Old-style response without protocol_type field
        old_style_toon = """task_slug=create_checkout_session
input_entities=[Customer]
output_entities=[Session]
constraints.idempotency_required=true
target_operations=[{operation_id:createSession,method:POST,path:/v1/sessions}]"""
        mock_llm.return_value = old_style_toon
        ...
        assert result.integration_task is not None
        assert result.integration_task.task_slug == "create_checkout_session"
        
        # protocol_type should default to REST
        extra = result.integration_task.constraints.get("extra", {})
        assert extra.get("protocol_type") == "rest"
        
        # target_operations should still be populated
        assert len(extra.get("target_operations", [])) == 1
        assert extra["target_operations"][0]["method"] == "POST"

    @patch("integration_coworker.graph.nodes.understand_task.call_llm_async_for_node")
    @pytest.mark.asyncio
    async def test_response_with_protocol_type_file(self, mock_llm):
        """LLM responses with protocol_type=file are honored."""
        file_toon = """task_slug=parse_transactions
input_entities=[TransactionRecord]
output_entities=[ParsedTransaction]
protocol_type=file
constraints.idempotency_required=false
target_operations=[{file_spec_id:1,op:parse}]"""
        mock_llm.return_value = file_toon
        
        state = make_state(task_description="Parse CSV transactions")
        result = await understand_task(state)
        
        assert result.integration_task is not None
        extra = result.integration_task.constraints.get("extra", {})
        assert extra.get("protocol_type") == "file"
